...
def initialize_trading():
    ...
    strategy_config = MartingaleConfig(
        ...
        price_interval=config.get('PRICE_INTERVAL', 0.006),
        max_dca_count=config.get('MAX_DCA_COUNT', 7),
            take_profit=config.get('TAKE_PROFIT', 0.01),
...
async def update_config(request: Request):
    ...
    cfg.update({
        ...
        'PRICE_INTERVAL': body.get('price_interval', 0.006),
        'MAX_DCA_COUNT': body.get('max_dca_count', 7),
        'TAKE_PROFIT': body.get('take_profit', 0.01),
